...
function siteConfiguration(config: SiteConfiguration): Plugin {
  ...
  const title = config.title ?? "The Legal Review — Book Management"
 ...
 * a preview-host script drives directly. The page exposes a
 * registry of every file matching `storiesGlob` on
 * window.__FIGMA__.stories so the preview surface can dynamically
